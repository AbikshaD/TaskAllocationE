    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          <h2 className="text-base font-semibold text-white">Review Project</h2>
        ...
        <div className="p-5 space-y-4">
          <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
            ...
            <p className="text-xs text-slate-400 mt-2">{item.department}</p>
            <p className="text-xs text-slate-500 mt-1">Student: <span className="text-slate-300">{item.allocatedTo?.name}</span> ({item.allocatedTo?.studentId})</p>
          </div>
          <div>
            <label className="label">Feedback</label>
            <textarea className="input resize-none" rows={3} value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="Project review feedback..." />
          </div>
          <div>
...
export default function AdminProjects() {
  ...
  const handleApprove = async (id, status, feedback, marks) => {
    const res = await api.put(`/tasks/projects/${id}/approve`, { status, adminFeedback: feedback, obtainedMarks: marks });
  ...
  return (
    <div className="flex-1 p-8 overflow-auto">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[['Total',counts.all,'text-white'],['Allocated',counts.allocated,'text-amber-400'],['Submitted',counts.submitted,'text-blue-400'],['Approved',counts.approved,'text-emerald-400']].map(([l,v,c]) => (
      ...
      <div className="table-container">
        <table className="table">
          <thead><tr><th>Project Title</th><th>Department</th><th>Student</th><th>Due</th><th>Marks</th><th>Status</th><th>Action</th></tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={7} className="text-center py-10 text-slate-500">Loading...</td></tr>
              : filtered.length === 0 ? <tr><td colSpan={7} className="text-center py-10 text-slate-500">No projects found</td></tr>
              : filtered.map(a => (
                <tr key={a._id}>
                  <td>
                    <p className="font-medium text-slate-200 max-w-[160px] truncate">{a.title}</p>
                    {a.description && <p className="text-xs text-slate-500 truncate max-w-[160px]">{a.description}</p>}
                  </td>
                  <td className="text-xs text-slate-400">{a.department || '—'}</td>
                  <td>
                    {a.allocatedTo ? <><p className="text-sm text-slate-200">{a.allocatedTo.name}</p><p className="text-xs text-slate-500">{a.allocatedTo.studentId}</p></> : <span className="text-xs text-slate-500">—</span>}
                  </td>
                  <td className="text-xs text-slate-400">{a.dueDate ? new Date(a.dueDate).toLocaleDateString() : '—'}</td>
                  <td className="text-xs text-slate-400">{a.obtainedMarks != null ? `${a.obtainedMarks}/${a.maxMarks}` : `—/${a.maxMarks}`}</td>
